...
import { useAsRef } from "@/shared/lib/hooks/use-as-ref";
import { useIsomorphicLayoutEffect } from "@/shared/lib/hooks/use-isomorphic-layout-effect";
import { useLazyRef } from "@/shared/lib/hooks/use-lazy-ref";
import {
  Popover,
  PopoverAnchor,
  PopoverContent,
    ...
    <TimePickerInputGroupContext.Provider value={inputGroupContextValue}>
      <PopoverAnchor asChild>
        <InputGroupPrimitive
          ...
          {...inputGroupProps}
          className={cn(
            "flex h-8 w-full cursor-text items-center gap-0.5 rounded-lg border border-input bg-transparent px-2.5 py-1 shadow-xs outline-none transition-colors dark:bg-input/30",
  ...
  return (
    <TimePickerGroupContext.Provider value={groupContextValue}>
      <PopoverContent
        ...
        {...contentProps}
        className={cn(
          "flex w-auto max-w-none flex-row p-0",
          className,
        )}
